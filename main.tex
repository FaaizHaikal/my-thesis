...
longer ranges, reduces the overall MAE from 36.90~cm to 2.24~cm, and removes the
operational dependence on active object tracking.
\end{abstract}
...
\begin{figure*}[h]
\centering
\includegraphics[width=0.72\textwidth]{static_ipm_evaluation_2.png}
...
$E_r = 0.0005$ deg/cm,
$K_\theta = 0.1994$ deg$^2$/cm,
$E_t = -1.6902\%$, and
$K_\rho = 0.042551$ cm.

The obtained parameters indicate that both systematic and non-systematic
odometry errors remain relatively small over the evaluated 1~m walking
trajectory. In particular, the low rotational bias suggests that the
robot maintains a consistent heading during forward walking, while the
small translational bias indicates only minor deviation from the
expected traveled distance. These results are consistent with the
observed walking behavior and suggest that the odometry system provides
a sufficiently accurate pose estimate for transforming IPM outputs into
global field coordinates during the experiment.
...
absolute distance errors.Comparative performance profiling between the legacy
regression-based.}
...
To evaluate the proposed IPM approach, a comparison was conducted with
the regression-based distance estimation method previously used by the
ICHIRO ITS team. The evaluation was performed in a static setup with
the ball placed at distances ranging from 20 cm to 400 cm. Both methods
estimated the ball distance while the robot actively tracked the target
to ensure consistent observations.

The resulting distance estimates and absolute errors are presented in Figure~\ref{fig:regression_vs_ipm},
while the aggregate error metrics are summarized in Table~\ref{tab:regression_vs_ipm}.
...
continuous object tracking and can estimate positions directly from
individual object detections, providing greater flexibility for
perception tasks in dynamic soccer environments.
...
A comparison with the regression-based method previously used by the
ICHIRO ITS team showed a substantial improvement in distance estimation
accuracy, reducing the MAE from 36.90~cm to 2.24~cm and the RMSE from
61.00~cm to 2.55~cm. Unlike the regression-based approach, the proposed
method does not depend on active object tracking or repeated empirical
distance calibration, making it more suitable for deployment in dynamic
humanoid soccer environments.
