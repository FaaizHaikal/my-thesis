...
This paper presents the implementation of IPM to estimate object distance on a humanoid soccer robot
...
show that the system produces stable object position estimation. In addition, a comparison with 
a previously used regression-based method by ICHIRO ITS demonstrates that the proposed IPM approach provides 
more consistent accuracy across a wider distance range and eliminates the dependency on active 
object tracking.
\end{abstract}
...
objects positions by projecting image points by assuming that objects of interest lie on a planar surface, \cite{Li2024}.
...
Previously, the ICHIRO ITS team utilized a polynomial regression-based 
...
The pixel $(u, v)$ in image plane can be converted to three-dimensional coordinate in 
...
\begin{figure}[!h]
\centering
\includegraphics[width=3in,height=2.25in]{camera-calibration.png}
...
All experiments were done on a $6 \times 9$ m kid-size field 
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.5\textwidth,height=2.5in]{Real Setup.jpg}
...
This behavior is inherent to the IPM formulation. In this setup, the robot's head 
is oriented forward with a tilt angle of $0^\circ$. As the object 
moves farther away, its image projection approaches the horizon line due to 
perspective effects. In this region, small variations in pixel location 
lead to large changes in the estimated depth, causing the value computed 
in Equation~\ref{eq:zc_final} to increase significantly and become more 
sensitive to noise.
...
locations during walking. Additionally, the goalpost ($O_6$) is excluded due 
ambiguity where only a single post instead of the complete goal may be detected.
This ambiguity makes it difficult to associate detections with a unique ground-truth reference. 
...
\midrule
$O_1$ & (555.3, 343.7) & 11.6 & (5.3, -6.3) \\
$O_2$ & (652.7, 202.3) & 18.5 & (2.7, 2.3) \\
$O_3$ & (743.5, 300.1) & 14.1 & (-6.5, 0.1) \\
\bottomrule
\end{tabular}
\end{table}
...
of uncertainty such as body oscillation, head movement, and detection noises.
Despite these challenges, the IPM method maintains relatively stable 
performance across all objects.

\subsection{Comparison with Regression-Based Method}

To evaluate the effectiveness of the proposed IPM approach to ICHIRO ITS team, 
...
for comparison. The experimental setup during these measurements are 
shown in Figure~\ref{fig:regression_vs_ipm_setup} while 
Figure~\ref{fig:regression_vs_ipm} shows the comparison between the 
ground-truth distance and the estimated distances produced by both methods.

\begin{figure}[h]
\centering
\includegraphics[width=0.5\textwidth,height=3in]{regression_vs_ipm_setup.jpg}
\caption{Actual experimental setup for comparing between regression-based and IPM distance estimation.}
\label{fig:regression_vs_ipm_setup}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.5\textwidth]{regression_vs_ipm.png}
\caption{Comparison between regression-based distance estimation and 
IPM across varying object distances.}
...
the estimated distance saturates and no longer follows the ground-truth trend.
...
the ground-truth distance across the entire range. Even at longer distances, 
...
capabilities such as localization.
...
dependency on active object tracking. In addition, it removes the need for repeated 
...
detection noise, joint offsets, and IMU drifts.
