...
\begin{tabular}{c c c}
\hline
Symbol & Object & Global Position $(X_i, Y_i)$ [m] \\
\hline
$O_1$ & Goalpost 1 & $(X_1, Y_1)$ \\
$O_2$ & Goalpost 2 & $(X_2, Y_2)$ \\
$O_3$ & Penalty Marker & $(X_3, Y_3)$ \\
$O_4$ & Soccer Ball & $(X_4, Y_4)$ \\
$O_5$ & Opponent Robot & $(X_5, Y_5)$ \\
\hline
\end{tabular}
\end{table}
...
constant for both static and dynamic experiments. A visualization of 
the experimental configuration is shown in Fig.~\ref{fig:experiment_setup}.
...
and the fixed global positions of objects $O_1$–$O_5$ on the field.}
...
position $P(X_0, Y_0)$, and both the body and head are kept fixed. 
...
The position error is computed as the Euclidean distance between 
the estimated and ground-truth object positions:

\begin{equation}
e_i = \sqrt{( \hat{X}_i - X_i )^2 + ( \hat{Y}_i - Y_i )^2}
\end{equation}

Table~\ref{tab:static_results} presents an example format of 
the static evaluation results.
...
\begin{tabularx}{\columnwidth}{@{} Y Y c @{}} % Using Y for Object and Position
\toprule
\textbf{Object} & \textbf{Estimated Position (m)} & \textbf{Error (m)} \\
\midrule
$O_1$ & (0.52, -0.01) & 0.02 \\
$O_2$ & (1.03, 0.18) & 0.04 \\
$O_3$ & (1.56, -0.28) & 0.06 \\
...
position $P(X_0, Y_0)$ and walks toward a predefined target point 
$Q(X_t, Y_t)$.
...
estimation error and odometry drift.

To characterize odometry performance independently, the robot 
repeatedly walks from $P(X_0, Y_0)$ to $Q(X_t, Y_t)$ for $K$ trials. 
After reaching the target, the odometry error is computed as:
\[
\Delta x = x_{\text{odom}} - x_{\text{true}}, \quad
\Delta y = y_{\text{odom}} - y_{\text{true}}, \quad
\Delta \theta = \theta_{\text{odom}} - \theta_{\text{true}}
\]

Following \cite{Martinelli2001}, odometry errors are statistically 
characterized in terms of mean and variance and categorized as:

\begin{itemize}
    \item Systematic errors (repeatable and trajectory-dependent),
    \item Non-systematic errors (random and externally influenced).
\end{itemize}

For each trial, IPM estimates are computed continuously for all 
visible objects, and the reported values correspond to the mean 
estimated position and mean Euclidean error over the duration 
of the walk. Odometry drift values are reported once per trial, 
as they equally affect all global reconstructions within that trial.

Table~\ref{tab:dynamic_results} presents an example format 
for the dynamic evaluation results.

\begin{table}[h]
\centering
\small
\setlength{\tabcolsep}{3pt} 
\caption{Dynamic IPM results with odometry drift per trial}
\label{tab:dynamic_results}
% Changed Object column to 'c' and kept Mean Pos as 'Y'
\begin{tabularx}{\columnwidth}{@{} c c Y c c c c @{}} 
\toprule
\textbf{Trial} & \textbf{Obj.} & \textbf{Mean Pos. (m)} & \textbf{Err. (m)} & \boldmath$\Delta x$ & \boldmath$\Delta y$ & \boldmath$\Delta \theta$ \\
\midrule
\multirow{5}{*}{1}
 & $O_1$ & (1.08, -0.02) & 0.10 & \multirow{5}{*}{0.05} & \multirow{5}{*}{-0.03} & \multirow{5}{*}{0.04} \\
 & $O_2$ & (1.62, 0.24)  & 0.13 &  &  &  \\
 & $O_3$ & (2.11, -0.28) & 0.11 &  &  &  \\
 & $O_4$ & (1.62, 0.24)  & 0.13 &  &  &  \\
 & $O_5$ & (2.11, -0.28) & 0.11 &  &  &  \\
\midrule
\multirow{5}{*}{2}
 & $O_1$ & (1.05, -0.01) & 0.08 & \multirow{5}{*}{0.04} & \multirow{5}{*}{-0.02} & \multirow{5}{*}{0.03} \\
 & $O_2$ & (1.57, 0.23)  & 0.09 &  &  &  \\
 & $O_3$ & (2.06, -0.27) & 0.07 &  &  &  \\
 & $O_4$ & (1.57, 0.23)  & 0.09 &  &  &  \\
 & $O_5$ & (2.06, -0.27) & 0.07 &  &  &  \\
