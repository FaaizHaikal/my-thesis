...
\end{minipage}
\caption{Complete experimental configuration deployed on the RoboCup field, displaying the robot starting kickoff orientation and the fixed global coordinates for targets $O_1$--$O_6$.}
\label{fig:combined_setup}
\end{figure*}


To assess the accuracy of the IPM estimations, three primary error metrics
are defined for each observed object: the estimated object position $(\hat{X}, \hat{Y})$
is compared with the corresponding ground-truth position $(X, Y)$. The estimation
error along each axis is defined as

\begin{equation}
E_x = \hat{X} - X, \quad
E_y = \hat{Y} - Y
\end{equation}
and the Euclidean position error is computed as
\begin{equation}
E = \sqrt{(E_x )^2 + ( E_y )^2}
\end{equation}

The Euclidean error represents the geometric distance between the estimated and
reference positions in the field coordinate system. All position coordinates and error
metrics reported in this study are expressed in centimeters (cm). For visualization purposes,
the signed axis-wise errors $(E_x, E_y)$ are retained to indicate
the direction of estimation bias.

For each object, multiple observations are collected during the evaluation process.
The distribution of Euclidean errors is summarized using the Mean Absolute Error (MAE),
Root Mean Square Error (RMSE), median error, Median Absolute Deviation (MAD), and standard
deviation ($\sigma$). In addition, 95\% confidence intervals for MAE and RMSE are estimated
using bootstrap resampling with 10,000 iterations.
...
1.61~cm (95\% CI: [1.52, 1.69]), while $O_6$ exhibits the largest error with
an MAE and RMSE of 38.80~cm. A clear increase in estimation error can be observed
as the object distance from the robot increases. This trend is particularly
evident for $O_5$ and $O_6$, which correspond to the most distant targets in
...
\begin{figure*}[h]
\centering
\includegraphics[width=0.6\textwidth]{static_ipm_evaluation_2.png}
...
the resulting position estimates exhibit higher variance than those of the
other evaluated targets.
...
\centering
\footnotesize
\caption{Shapiro--Wilk normality test results for dynamic evaluation errors}
...
To formally assess the normality of the dynamic error distributions,
the Shapiro--Wilk normality test was performed for each evaluated object.
The results are presented in Table~\ref{shapiro_wilk_test}. All
evaluated objects produce extremely small p-values ($p < 0.05$),
indicating that the null hypothesis of a normally distributed error
population must be rejected.

To further visualize these deviations from normality, Quantile--Quantile
(Q--Q) plots were generated for each object against a theoretical normal
distribution, as shown in Figure~\ref{fig:dynamic_qq_plots}.

\begin{figure*}[h]
\centering
\includegraphics[width=\textwidth]{dynamic_errors_qq_plots.png}
\caption{Quantile-Quantile (Q-Q) plots of Euclidean distance errors for the ball ($O_1$), robot ($O_2$), and X-intersection ($O_3$) during dynamic walking trials. The sharp upward curve away from the straight normal line shows that the errors do not follow a normal distribution, but are heavily right-skewed due to occasional large estimation spikes.}
\label{fig:dynamic_qq_plots}
\end{figure*}

The visual trends in Figure~\ref{fig:dynamic_qq_plots} are consistent with the
Shapiro--Wilk test results. For all three objects, the data points bend sharply
upward away from the linear normal baseline, particularly in the upper quantiles.
This distinct upward curvature demonstrates that the tracking errors do not follow
a Gaussian distribution, but are heavily right-skewed by occasional large estimation
spikes.

Together, the statistical tests and Q--Q plots confirm that the dynamic
errors are strongly non-normal. Consequently, robust metrics like the median
and MAD provide a much more accurate description of the system's typical
tracking performance than standard mean-based metrics alone.
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.6\textwidth]{dynamic_ipm_evaluation_2.png}
\caption{Dynamic IPM evaluation in field coordinates during active walking, displaying spatial distribution arrays, true reference markers, and the designated 15~cm error tolerance boundaries for targets $O_1$, $O_2$, and $O_3$.}
\label{fig:dynamic_ipm_evaluation}
\end{figure*}

Several failure cases were observed during the evaluation, particularly when
the estimated object position was projected to an unrealistic location on the field.
Since IPM computes real-world coordinates directly from pixel coordinate,
small errors in object detection, camera pose, or calibration can produce
large position deviations, especially for distant objects and image regions near the horizon.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.72\columnwidth]{regression_vs_ipm_setup.jpg}
...
\toprule
\textbf{Method} & \textbf{MAE} & \textbf{RMSE} & \textbf{Max. Error} \\
\midrule
...
\end{minipage}
\caption{Comparative performance profiling between the legacy regression-based infrastructure and the proposed analytical IPM architecture: (a) estimated distance curves plotted against ground-truth references, and (b) corresponding absolute distance errors across the 400~cm operational target line.}
\label{fig:regression_vs_ipm}
\end{figure*}
As shown in Figure~\ref{fig:regression_vs_ipm}a, the regression-based method
...
continuous object tracking, as demonstrated in both the dynamic
