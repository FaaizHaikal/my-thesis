...
One benchmark for evaluating these advancements is robotic soccer, such as the RoboCup Humanoid League,
where robots must perceive their environment, make decisions, and move autonomously
in real time \cite{RoboCupHumanoidLeague2025}.

Humanoid soccer robot must estimate the positions of objects such as the ball, goalposts, 
and field features to perform navigation and make decisions \cite{SatyaWidodo2012}. Several approaches 
have been proposed for estimating object distance using visual data. Monocular Depth Estimation uses
...
In this paper, we implemented Inverse Perspective Mapping (IPM), a geometric method that estimates
objects positions by projecting image points onto a known ground plane \cite{Li2024}.
...
to body posture changes \cite{Dias2023}.

IPM relies on the \textit{pinhole camera model}, which describes the relation of three-dimensional
world point and two-dimensional image plane as shown in Equation \ref{eq:1}.
...
By assuming that objects of interests lie on a planar surface, the inverse of Equation \ref{eq:1} can be used to recover
the spatial position of objects relative to the world frame \cite{Hartley2004,Mark2024}.

The remaining of the paper is organized as follows: Section II describes the proposed IPM approach and its
integration with the robot kinematic model. Section III presents experimental results and analysis.
Section IV concludes the paper.

\section{Methodology}

This section describes the implemented vision-based distance estimation using
IPM on a humanoid soccer robot. The methodology consist of camera calibration,
kinematic modeling of the robot, and IPM implementation to estimate object distance.
...
image plane as mentioned in Equation \ref{eq:1}. In this paper, we utilized a Logitech C920 camera that was mounted
on the robot's head.
...
various distances and angles as shown in Figure \ref{fig:1}.
...
The robot's physical structure is modeled using the Universal Robot Definition Format
(URDF). The model contains 5 static frames and 20 robot frames that forms a hierarchical tree.
A simplified representation of this hierarchical tree is illustrated in Figure \ref{fig:2}.

\begin{figure}[!h]
\centering
\includegraphics[width=\linewidth]{Simplified Kinematic Tree.png}
\caption{Simplified kinematic tree of the humanoid robot.}
\label{fig:2}
\end{figure}

The \texttt{camera\_frame} follows the standard ROS vision axis where the $Z$-axis points 
forward along optical axis, $Y$-axis points downward, and $X$-axis points to the right \cite{IsaacSimDocumentation}.

We implemented \texttt{robot\_frames}, a ROS 2 package which loads the URDF and
broadcasts kinematic transformations using the \texttt{tf2} library. By consuming live joint encoders 
and IMU data, the package provides real-time kinematic transformations
that reflects the robot's physical state in the real world.

Additionally, the package calculates \texttt{base\_footprint}, a virtual frame
that serves as the reference frame in IPM calculation. The frame is calculated dynamically using
\texttt{right\_foot\_frame} and \texttt{left\_foot\_frame} information. Since the robot will 
always be on the ground, the $Z$-height of the \texttt{base\_footprint} is set to be the same
as the supporting foot (the foot in contact with the ground). Meanwhile, the $X$ and $Y$ coordinates
are determined by the midpoint between both feet.

\subsection{Inverse Perspective Mapping}

We implemented a dedicated ROS 2 package to perform distance estimation. 
This package subscribes to object bounding boxes from a Deep Neural Network (DNN) 
detection module and kinematic transformations from the \texttt{robot\_frames} package. 
Additionally, it loads a JSON configuration file containing the camera intrinsic 
parameters ($K$) derived from the calibration process.

Detected objects are represented as bounding boxes in the image plane. 
To estimate distance, a specific pixel $(x, y)$ must be selected that represent 
the object's contact point with the ground. For objects such as other robots or goalposts, 
the bottom-center of the bounding box is selected. For objects such as the ball or field markers, 
the center of the bounding box is used.

Note that for certain objects, such as ball, the target pixel may not represent direct ground contact. 
This discrepancy is addressed by a height offset ($h$), which is explained later in this section.

The first step in the deprojection process is to convert the target pixel $(x, y)$ 
into three-dimensional coordinate in camera frame $P_c = [X_c, Y_c, Z_c]^T$. 
Using the intrinsic parameters, $X_c$ and $Y_c$ can be expressed as shown in Equation \ref{eq:2}.
...
To recover $Z_c$, We define a plane in the \texttt{camera\_frame} using plane equation as shown in Equation \ref{eq:3}.

\begin{equation} \label{eq:3}
n_c \cdot P_c + d = 0
\end{equation}
where $n_c = [n_x, n_y, n_z]^T$ is the plane normal in the camera frame and $d$ 
is the perpendicular distance from the camera origin to the plane. By substituting 
$P_c = [u Z_c, v Z_c, Z_c]^T$ into Equation \ref{eq:3}, we solve for $Z_c$:

\begin{equation} \label{eq:4}
Z_c = \frac{-d}{n_x u + n_y v + n_z}
\end{equation}

The normal $n_c$ is derived by transforming the fixed ground normal from the \texttt{base\_footprint} 
...
By applying the rotation matrix $R_{b}^{c}$, we can express this normal in the camera's coordinate system:

\begin{equation} \label{eq:5}
n_c = R_{b}^{c} \cdot n_b = \begin{bmatrix} r_{13} \\ r_{23} \\ r_{33} \end{bmatrix}
\end{equation}
where $r_{13}, r_{23}, r_{33}$ are elements of the third row of the rotation matrix $R_{b}^{c}$.

The distance $d$ from the plane in the \texttt{camera\_frame} is determined by 
the translation $T_z$ of the camera along the $Z$-axis relative to the base frame. 
However, we must account for objects where the target pixel does not represent 
direct contact with the ground, such as a ball. In these cases, we introduce a height offset $h$, 
which effectively treats the ground as being elevated by that value. For example, since the 
ball's dimensions are known, the height offset $h$ corresponds to its radius. 
Hence, we can define $d$ as:

\begin{equation} \label{eq:6}
d = -(T_z - h)
\end{equation}

Substituting the normal $n_c$ and the distance $d$ into Equation \ref{eq:4} gives us 
the final depth calculation:
