...
\thanks{Manuscript received April XX, 2026; revised April XX, 2026. This
...
because the camera extrinsic parameters change continuously during walking and 
head movement, IPM is challenging to implement reliably. 

This paper presents an IPM implementation for object distance estimation on a humanoid soccer robot
developed by ICHIRO ITS team using ROS 2 software, a widely used robotics framework that enables modular 
and real-time data exchange between multiple robot subsystems, 
including sensors and processing modules. The proposed system integrates 
object detection outputs, camera intrinsic parameters, and real-time kinematic transformations 
obtained from the robot model. Evaluations under static and walking conditions 
show that the system produces stable object position estimates. In addition, a comparison with 
a previously used regression-based method by ICHIRO ITS demonstrates that the proposed IPM approach provides 
more consistent accuracy across a wider distance range and removes the dependence on active 
object tracking.
\end{abstract}
...
One benchmark for evaluating these advancements is competitive soccer, which tests autonomous systems 
...
single camera remains a critical and challenging problem \cite{RoboCupHumanoidLeague2025}. Humanoid soccer robots must estimate 
...
by defining a plane in the camera frame $P_c$ using plane equation as shown in Equation \ref{eq:plane_camera}
...
\end{equation}
where $r_{31}, r_{32}, r_{33}$ are elements of the third row of the rotation matrix $R_{c}^{b}$.
...
from Equation \ref{eq:pinhole}:

\begin{equation} \label{eq:coordinate_camera} 
\begin{bmatrix} X_c \\ Y_c \end{bmatrix} = Z_c 
\begin{bmatrix} \frac{u - p_x}{f_x} \\ \frac{v - p_y}{f_y} \end{bmatrix} = Z_c 
...
\begin{figure}[!h]
\centering
\includegraphics[width=\columnwidth]{camera-calibration.png}
...
equation derived in Section \ref{sec:theories} is modified as follows:
...
In addition, a comparative evaluation is conducted against the 
previous regression-based method used in the system. This comparison 
highlights the limitations of the prior approach and demonstrates 
the advantages of the proposed IPM method in terms of scalability 
and robustness across varying distances.
...
experimental configuration is shown in Figure~\ref{fig:experiment_setup}, 
while the actual experimental setup is presented in Figure~\ref{fig:real_setup}.
...
\begin{figure}[!h]
\centering
\includegraphics[width=\columnwidth]{Real Setup.jpg}
...
error ($E_x$), and the Y-axis error ($E_y$). These errors represent the overall 
geometric deviation between the estimated position $(\hat{X}, \hat{Y})$ and the reference position 
$(X, Y)$, defined as:
...
\begin{equation}
E^{\%} = \frac{E}{\sqrt{(X - X_0)^2 + (Y - Y_0)^2}} \times 100\%
\end{equation}
...
The signed axis-wise errors $(E_x, E_y)$ are retained to indicate the 
direction of estimation bias, while absolute values are used in the 
percentage formulation to represent error magnitude independently of direction.

For evaluations involving multiple observations or repeated trials, 
additional statistical metrics are used to analyze the overall estimation 
performance. The Mean Absolute Error (MAE) represents the average magnitude 
of the estimation error, the Root Mean Square Error (RMSE) emphasizes larger 
deviations caused by outliers, and the standard deviation ($\sigma$) measures 
the variability of the estimation results.
...
\end{equation}
where $N$ denotes the number of evaluated samples, $E_i$ is the 
Euclidean error of the $i$-th sample, and $\bar{E}$ is the mean 
Euclidean error.
...
Since the robot pose is known and does not change during this evaluation, 
it is used as the reference pose.
...
\midrule
$O_1$ & (550.0, 348.4) & 1.6 (1.4\%) & 0.0 (0.0\%) & -1.6 (3.1\%) \\
$O_2$ & (644.0, 197.3) & 6.6 (3.0\%) & -6.0 (3.0\%) & -2.7 (2.7\%) \\
$O_3$ & (746.7, 300.5) & 3.3 (1.1\%) & -3.3 (1.1\%) & 0.5 (0.0\%) \\
$O_4$ & (806.2, 446.8) & 7.0 (1.9\%) & 6.2 (1.8\%) & -3.2 (2.2\%) \\
$O_5$ & (916.4, 463.4) & 21.3 (4.5\%) & 16.4 (3.7\%) & 13.4 (8.9\%) \\
$O_6$ & (938.6, 194.3) & 38.8 (8.4\%) & 38.6 (8.6\%) & 4.3 (3.9\%) \\
\bottomrule
\end{tabular}
\end{table}

The results in Table~\ref{tab:static_results} show that the proposed 
IPM method achieves high accuracy for objects located at moderate distances, 
with most errors remaining below 10 cm. However, a clear trend can be observed 
where the estimation error increases as the object distance becomes larger, 
particularly for $O_5$ and $O_6$.

Figure~\ref{fig:static_ipm_evaluation} visualizes the same result in 
field coordinates by comparing the reference object positions with the 
IPM estimates. The error vectors show that the estimates remain close 
to the reference positions for nearby and mid-range objects, while the 
largest deviations occur for distant objects.

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{static_ipm_evaluation.png}
...
in Equation~\ref{eq:zc_final} becomes increasingly sensitive to pixel noise, 
segmentation inaccuracies, and calibration errors, resulting in larger 
...
distance between the estimated and reference object positions. 
...
Before computing the aggregate dynamic results, outliers caused by false 
or ambiguous object detections are removed using the interquartile range 
(IQR) criterion for each object class. Approximately 17\% of the collected samples were removed 
during this process. Most removed samples correspond to intermittent 
misdetections and blurred observations occurring during robot motion 
and head scanning. The filtered results are then used 
to evaluate the stability of the IPM estimates during walking.
...
\begin{tabular}{c c c c c}
\toprule
\textbf{Obj} & \textbf{MAE (cm)} & \textbf{RMSE (cm)} & \textbf{$\sigma$ (cm)} & \textbf{($E_x, E_y$) (cm)} \\
\midrule
$O_1$ & 11.56 & 16.12 & 11.23 & (5.29, -6.26) \\
$O_2$ & 18.47 & 33.44 & 27.88 & (2.72, 2.33) \\
$O_3$ & 14.14 & 50.17 & 48.13 & (-6.52, 0.08) \\
\bottomrule
\end{tabular}
\end{table}

The spatial distribution of the dynamic estimates is shown in 
Figure~\ref{fig:dynamic_ipm_evaluation}. A 15 cm tolerance circle is 
drawn around each reference object position to indicate the concentration 
of the filtered IPM estimates. The figure shows that most ball and X-intersection
estimates remain clustered around their reference positions, while 
the robot object has a wider spread because its bounding box changes more 
strongly with walking motion and detection variation.

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{dynamic_ipm_evaluation.png}
\caption{Dynamic IPM evaluation in field coordinates during walking, showing 
IPM estimates, reference positions, and 15 cm tolerance regions for 
the ball, robot, and X-intersection.}
\label{fig:dynamic_ipm_evaluation}
\end{figure}

A noticeable difference between the MAE and RMSE values can be observed 
for several objects, particularly $O_2$ and $O_3$. This indicates the 
presence of occasional large estimation errors during certain trials. 
Since RMSE squares the error term, it becomes more sensitive to outliers 
than MAE. These larger deviations are primarily caused by false detections, 
image blur during rapid head scanning due to the camera's rolling-shutter behavior, 
and inaccuracies in the camera extrinsic parameters while the robot is walking.
...
reference pose $(x_{\text{true}}, y_{\text{true}}, \theta_{\text{true}})$ 
at the end of each trial, when the robot reaches the target position 
$P'(550.0, 300.0)$. The resulting odometry error parameters are:
\begin{align*}
E_r &= 0.0005 \ \text{deg/cm}, \\
K_\theta &= 0.1994 \ \text{deg}^2/\text{cm}, \\
E_t &= -1.6902\%, \\
K_\rho &= 0.042551 \ \text{cm}.
\end{align*}

The relatively small magnitude of these parameters suggests that odometry 
drift remains limited during the evaluated walking distance. Nevertheless, 
its contribution cannot be completely separated from the overall dynamic 
estimation error, as the global coordinate transformation depends on the 
odometry-based robot pose estimate.

The dynamic evaluation results in Table~\ref{tab:dynamic_results} show an overall 
increase in estimation error compared to the static case. This is expected, 
as the robot performs continuous motion, introducing additional sources 
of uncertainty such as body oscillation, head movement, and detection noise.
Despite these challenges, the proposed IPM method maintains consistent 
estimation performance across all evaluated object classes.

\subsection{Comparison with Regression-Based Method}

To evaluate the effectiveness of the proposed IPM approach for the ICHIRO ITS team, 
a comparison is conducted with the previous distance estimation method used in the 
system, which is based on polynomial regression of the robot head 
pan and tilt angles.

The evaluation is performed in a static setup, where the ball is placed 
at increasing distances from the robot, ranging from 20 cm to 400 cm. 
For each distance, both the regression-based method and the proposed 
IPM method are used to estimate the distance to the ball. In both approaches, 
the robot actively tracks the ball to ensure a consistent observation 
for comparison. The experimental setup during these measurements is 
shown in Figure~\ref{fig:regression_vs_ipm_setup}. 

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{regression_vs_ipm_setup.jpg}
...
The distance estimates and absolute errors are shown in Figure~\ref{fig:regression_vs_ipm}, while 
the aggregate error metrics are summarized in Table~\ref{tab:regression_vs_ipm}.

\begin{table}[h]
\centering
\footnotesize
\caption{Distance estimation error summary for regression and IPM}
\label{tab:regression_vs_ipm}
\begin{tabular}{c c c c}
\toprule
\textbf{Method} & \textbf{MAE (cm)} & \textbf{RMSE (cm)} & \textbf{Max. Error (cm)} \\ 
\midrule
Regression-based & 36.90 & 61.00 & 154.60 \\
IPM & 2.24 & 2.55 & 4.20 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.88\columnwidth]{regression_vs_ipm.png}
\caption{Comparison between regression-based and IPM distance estimation: 
(a) estimated distance against reference distance and (b) absolute error 
across the tested distance range.}
\label{fig:regression_vs_ipm}
\end{figure}

As shown in Figure~\ref{fig:regression_vs_ipm}, the regression-based method 
produces acceptable performance at short distances but fails to generalize 
as the distance increases. In particular, beyond approximately 250 cm, 
the estimated distance saturates and no longer follows the reference trend.
This limitation arises because the regression model relies primarily on the head 
tilt angle as its input feature. As the object moves farther away, the tilt angle 
eventually remains at $0^\circ$, reducing the sensitivity of the 
mapping and causing the model to lose its predictive capability.

In contrast, the proposed IPM method maintains a consistent relationship with 
the reference distance across the entire range. Even at longer distances, 
the estimated values continue to increase proportionally, demonstrating better 
scalability and robustness. The quantitative summary in 
Table~\ref{tab:regression_vs_ipm} further shows that IPM 
substantially reduces the average and worst-case distance estimation errors 
under the tested conditions.

Furthermore, unlike the regression-based approach, IPM does not require 
continuous object tracking, as demonstrated in both the static and dynamic 
evaluations. This enables more flexible perception and supports higher-level 
capabilities such as localization, which is critical for maintaining the 
robot's spatial awareness and tactical positioning on the field.
...
Experimental results demonstrate that the system achieves reliable performance under 
both static and dynamic conditions. In static scenarios, the method produces accurate 
and consistent estimates across multiple object types. During walking, the system 
maintains stable performance despite the influence of robot motion and odometry drift, 
showing its robustness in realistic operating conditions.

Compared to the previously used regression-based method by ICHIRO ITS, the proposed IPM approach 
provides more consistent accuracy over a wider distance range and eliminates the 
dependence on active object tracking. In addition, it removes the need for repeated 
empirical calibration, making it more practical for deployment in dynamic and 
time-constrained environments such as humanoid robot competitions.
